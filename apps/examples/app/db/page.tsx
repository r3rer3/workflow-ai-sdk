 ...
 * Frontend for `/api/db`.
 *
 * `/api/db` is a Supabase-backed resumable workflow gated by RLS. The
 * pedagogical sign-in helper at `/api/auth` writes a Supabase session into
 * cookies, so we POST to that first to set the cookie, then drive the
 * workflow exactly the same way as the in-memory `/resume` page.
 *
 * Errors are routed through component state so we can surface 401, 404,
 * and 503 responses with explanations rather than a generic "fetch
 * failed".
 */
...
type DbWorkflowPart = DbWorkflowMessage["parts"][number];

function findPendingPause(
  parts: readonly DbWorkflowPart[],
): PendingPause | null {
...
export default function DbPage() {
  ...
  const { messages, sendMessage, status, error } = useChat<DbWorkflowMessage>({
    transport: new DefaultChatTransport({
      api: "/api/db",
      prepareSendMessagesRequest: ({ body }) => ({
        body: body ?? { action: "start", topic: "quick-persistence-demo" },
      }),
    }),
  });
  ...
  const handleSignIn = async (event: SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setSignInError(detail?.error ?? `Sign-in failed (${response.status}).`);
        return;
      }
      setSignedIn(true);
      setPassword("");
    ...
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col gap-6 bg-background p-6 text-foreground">
        ...
        >
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-muted-foreground">
              Replace with your real auth flow in production.
            </span>
            <button
              type="submit"
      ...
      <Conversation className="min-h-[320px] flex-1 overflow-y-auto rounded-xl border bg-card/50">
        <ConversationContent>
          {messages.length === 0 && (
            <ConversationEmptyState
              title={signedIn ? "Start a persisted run" : "Sign in first"}
              description={
                signedIn
                  ? "The workflow row will be scoped to your user_id under RLS."
                  : "The /api/db route requires a Supabase session cookie."
              }
            />
          )}
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-muted-foreground">
              {activePending
                ...
                : isStreaming
                  ? "Persisting…"
                  : "Submit to start a persisted run."}
            </span>
            <button
              type="submit"
              disabled={
